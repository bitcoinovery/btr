...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete BTR database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no BTR datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old BTR datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected BTR 0.7 datadir."
    ;;
  3)
    echo "Detected BTR pre-0.8 datadir."
    ;;
  4)
    echo "Detected BTR 0.8 datadir."
    ;;
esac
