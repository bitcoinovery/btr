...
*Setup instructions for a Gitian build of BTR Core using a Debian VM or physical system.*
...
- [Building BTR Core](#building-btr-core)
...
Clone the git repositories for BTR Core and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/btrpay/btr
```
...
in the BTR Core repository under 'Fetch and build inputs' to install sources which require
...
To build BTR Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the BTR Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/btr/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/btrpay/btr
...
For example:
```bash
URL=https://github.com/crowning-/btr.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit btr=${COMMIT} --url btr=${URL} ../btr/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit btr=${COMMIT} --url btr=${URL} ../btr/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit btr=${COMMIT} --url btr=${URL} ../btr/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the btr git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=precise on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../btr/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/btr.git
SIGPATH=/some/root/path/btr-detached-sigs.git

./bin/gbuild --url btr=${BTCPATH},signature=${SIGPATH} ../btr/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/btr-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/btr-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/btr-osx-build.assert
```
...
[btr/gitian.sigs](https://github.com/btrpay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
