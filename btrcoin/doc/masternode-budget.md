...
Format: ```mngovernance prepare proposal-name url payment-count block-start btr-address monthly-payment-btr```
...
Format: ```mngovernance submit proposal-name url payment-count block-start btr-address monthly-payment-btr fee-tx```
...
When block ```1000000``` is reached you'll receive a payment for ```1200``` BTR to ```y6R9oN12KnB9zydzTLc3LikD9cCjjQzYG7```.
...
The following RPC commands are supported:
 ...
 - mngovernance "command"... ( "passphrase" )
  ...
  - vote               - Vote on a proposal by single masternode (using btr.conf setup)
