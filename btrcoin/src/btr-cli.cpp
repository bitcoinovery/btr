...
static int AppInitRPC(int argc, char* argv[])
{
    ...
    if (argc<2 || mapArgs.count("-?") || mapArgs.count("-h") || mapArgs.count("-help") || mapArgs.count("-version")) {
        std::string strUsage = _("BTR Core RPC client version") + " " + FormatFullVersion() + "\n";
        if (!mapArgs.count("-version")) {
            strUsage += "\n" + _("Usage:") + "\n" +
                  "  BTR-cli [options] <command> [params]  " + _("Send command to BTR Core") + "\n" +
                  "  BTR-cli [options] help                " + _("List commands") + "\n" +
                  "  BTR-cli [options] help <command>      " + _("Get help for a command") + "\n";
