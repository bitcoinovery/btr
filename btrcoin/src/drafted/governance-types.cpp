...
GovernanceObjectType GovernanceStringToType(std::string strType) {
    ...
    if(strType == "BTRNetwork") type = BTRNetwork;
    if(strType == "BTRNetworkVariable") type = BTRNetworkVariable;
...
std::string GovernanceTypeToString(GovernanceObjectType type) {
    ...
    if(type == BTRNetwork) s = "BTRNetwork";
    if(type == BTRNetworkVariable) s = "BTRNetworkVariable";
