...
*
*   Why is this needed?
*   - In BTR some blocks are superblocks, which output much higher amounts of coins
...
bool IsBlockPayeeValid(const CTransaction& txNew, int nBlockHeight, CAmount blockReward)
{
    ...
    /*if(nBlockHeight < consensusParams.nSuperblockStartBlock)*/ {
        if(mnpayments.IsTransactionValid(txNew, nBlockHeight)) {
            LogPrint("mnpayments", "IsBlockPayeeValid1 -- Valid masternode payment at height %d: %s", nBlockHeight, txNew.ToString());
            return true;
        }
        else
        {
            LogPrint("mnpayments","UnValid masternode payment at height %d: %s", nBlockHeight, txNew.ToString());
...
//    }
    ...
    if(mnpayments.IsTransactionValid(txNew, nBlockHeight)) {
        LogPrint("mnpayments", "IsBlockPayeeValid2 -- Valid masternode payment at height %d: %s", nBlockHeight, txNew.ToString());
        return true;
    }
    else
    {
    ...
    if(sporkManager.IsSporkActive(SPORK_8_MASTERNODE_PAYMENT_ENFORCEMENT)) {
        LogPrint("mnpayments","IsBlockPayeeValid -- ERROR: Invalid masternode payment detected at height %d: %s", nBlockHeight, txNew.ToString());
        return false;
    }

    LogPrint("mnpayments","IsBlockPayeeValid -- WARNING: Masternode payment enforcement is disabled, accepting any payee\n");
    return true;
}
...
void CMasternodePayments::FillBlockPayee(CMutableTransaction& txNew, int nBlockHeight, CAmount blockReward, CTxOut& txoutMasternodeRet)
{
    ...
    if(!mnpayments.GetBlockPayee(nBlockHeight, payee)) {
        ...
        if(!mnodeman.GetNextMasternodeInQueueForPayment(nBlockHeight, true, nCount, mnInfo)) {
            ...
            LogPrint("mnpayments","CMasternodePayments::FillBlockPayee -- Failed to detect masternode to pay\n");
            return;
        }
...
{
    if(fLiteMode) return; // disable all BTR specific functionality
...
bool CMasternodePayments::AddPaymentVote(const CMasternodePaymentVote& vote)
{
    ...
    LogPrint("mnpayments","addpaymentvote count:%d \n",vote.nBlockHeight);
    if(!GetBlockHash(blockHash, vote.nBlockHeight - 10))
    {
        LogPrintf("AddPaymentVote:getblockhash error \n");
       return false;
    }

    if(HasVerifiedPaymentVote(vote.GetHash()))
    {
        LogPrint("mnpayments","AddPaymentVote.. HasVerifiedPaymentVote \n");
        return false;
    }
...
bool CMasternodeBlockPayees::HasPayeeWithVotes(const CScript& payeeIn, int nVotesReq)
{
    ...
    BOOST_FOREACH(CMasternodePayee& payee, vecPayees) {
        LogPrint("mnpayments","HasPayeeWithVotes count:%d \n  getPayee():%s payeeIn:%s \n",payee.GetVoteCount(),HexStr(payee.GetPayee()),HexStr(payeeIn));
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransaction& txNew)
{
    ...
    BOOST_FOREACH(CMasternodePayee& payee, vecPayees) {
        LogPrint("mnpayments","payeeGetVoteCount:%d nMaxSignatures:%d\n",payee.GetVoteCount(),nMaxSignatures);
        if (payee.GetVoteCount() >= nMaxSignatures) {
            nMaxSignatures = payee.GetVoteCount();
        }
    }

    LogPrint("mnpayments","nMaxSignatures:%d MNPAYMENTS_SIGNATURES_REQUIRED:%d \n",nMaxSignatures,MNPAYMENTS_SIGNATURES_REQUIRED);
    ...
    BOOST_FOREACH(CMasternodePayee& payee, vecPayees) {
        if (payee.GetVoteCount() >= MNPAYMENTS_SIGNATURES_REQUIRED) {
            ...
            LogPrint("mnpayments","payeeGetVoteCount2:%d nMaxSignatures:%d\n",payee.GetVoteCount(),MNPAYMENTS_SIGNATURES_REQUIRED);
    ...
    LogPrint("mnpayments","CMasternodeBlockPayees::IsTransactionValid -- ERROR: Missing required payment, possible payees: '%s', amount: %f BTR\n", strPayeesPossible, (float)nMasternodePayment/COIN);
    return false;
}
...
bool CMasternodePayments::IsTransactionValid(const CTransaction& txNew, int nBlockHeight)
{
    ...
    if(mapMasternodeBlocks.count(nBlockHeight)){
        LogPrint("mnpayments","mapMasternodeBlocks.count find: %d\n",nBlockHeight);
        return mapMasternodeBlocks[nBlockHeight].IsTransactionValid(txNew);
    }

    LogPrint("mnpayments","mapMasternodeBlocks.count not  find: %d\n",nBlockHeight);
    return true;
}
...
void CMasternodePayments::CheckAndRemove()
{
    ...
    LogPrint("mnpayments","CMasternodePayments::CheckAndRemove -- %s\n", ToString());
}
...
bool CMasternodePaymentVote::IsValid(CNode* pnode, int nValidationHeight, std::string& strError, CConnman& connman)
{
    ...
    if(!mnodeman.GetMasternodeRank(vinMasternode.prevout, nRank, nBlockHeight - 61, nMinRequiredProtocol)) {
...
bool CMasternodePayments::ProcessBlock(int nBlockHeight, CConnman& connman)
{
    ...
    if (!mnodeman.GetMasternodeRank(activeMasternode.outpoint, nRank, nBlockHeight - 61, GetMinMasternodePaymentsProto())) {
...
void CMasternodePayments::CheckPreviousBlockVotes(int nPrevBlockHeight)
{
    ...
    if (!mnodeman.GetMasternodeRanks(mns, nPrevBlockHeight - 61, GetMinMasternodePaymentsProto())) {
    ...
    for (int i = 0; i < MNPAYMENTS_SIGNATURES_TOTAL && i < (int)mns.size(); i++) {
        ...
        {
            LogPrintf("mapMasternodeBlocks.count null\n");
        }
