...
void CMasternodeSync::SwitchToNextAsset(CConnman& connman)
{
    switch(nRequestedMasternodeAssets)
    {
        ...
        case(MASTERNODE_SYNC_INITIAL):
            ...
            LogPrint("mnpayments","CMasternodeSync::SwitchToNextAsset -- Starting %s\n", GetAssetName());
            break;
        case(MASTERNODE_SYNC_WAITING):
            ClearFulfilledRequests(connman);
            LogPrint("mnpayments","CMasternodeSync::SwitchToNextAsset -- Completed %s in %llds\n", GetAssetName(), GetTime() - nTimeAssetSyncStarted);
        ...
        case(MASTERNODE_SYNC_GOVERNANCE):
            ...
            LogPrint("mnpayments","CMasternodeSync::SwitchToNextAsset -- Sync has finished\n");
...
void CMasternodeSync::ProcessTick(CConnman& connman)
{
    ...
    LogPrint("mnpayments","CMasternodeSync::ProcessTick -- nTick %d nRequestedMasternodeAssets %d nRequestedMasternodeAttempt %d nSyncProgress %f\n", nTick, nRequestedMasternodeAssets, nRequestedMasternodeAttempt, nSyncProgress);
    ...
    BOOST_FOREACH(CNode* pnode, vNodesCopy)
    {
        ...
        // NORMAL NETWORK MODE - TESTNET/MAINNET
        {
            if(netfulfilledman.HasFulfilledRequest(pnode->addr, "full-sync")) {
                ...
                LogPrint("mnpayments","CMasternodeSync::ProcessTick -- disconnecting from recently synced peer %d\n", pnode->id);
                continue;
            }
            ...
            if(!netfulfilledman.HasFulfilledRequest(pnode->addr, "spork-sync")) {
                ...
                LogPrint("mnpayments","CMasternodeSync::ProcessTick -- nTick %d nRequestedMasternodeAssets %d -- requesting sporks from peer %d\n", nTick, nRequestedMasternodeAssets, pnode->id);
            }
            ...
            if(nRequestedMasternodeAssets == MASTERNODE_SYNC_LIST) {
                ...
                if(GetTime() - nTimeLastBumped > MASTERNODE_SYNC_TIMEOUT_SECONDS) {
                    LogPrint("mnpayments","CMasternodeSync::ProcessTick -- nTick %d nRequestedMasternodeAssets %d -- timeout\n", nTick, nRequestedMasternodeAssets);
