...
CMasternode::CollateralStatus CMasternode::CheckCollateral(const COutPoint& outpoint, int& nHeightRet)
{
    ...
    if(coin.out.nValue != 5000 * COIN) {//tbd
...
void CMasternode::Check(bool fForce)
{
    ...
    LogPrint("masternode", "waitforping:%d outmasternode:%d fMasterNode:%d \n",fWaitForPing,fOurMasternode,fMasterNode);
    ...
    if(!fWaitForPing || fOurMasternode) {
        ...
        LogPrint("masternode", "CMasternode::Check -- outpoint=%s, nTimeLastWatchdogVote=%d, GetAdjustedTime()=%d, fWatchdogExpired=%d fWatchdogActive=%d\n",
                vin.prevout.ToStringShort(), nTimeLastWatchdogVote, GetAdjustedTime(), fWatchdogExpired,fWatchdogActive);
    ...
    LogPrint("masternode", "lastPing:%s sigTime:%s \n",boost::lexical_cast<std::string>(lastPing.sigTime),boost::lexical_cast<std::string>(sigTime));
...
bool CMasternode::IsInputAssociatedWithPubkey()
{
    ...
    if(GetTransaction(vin.prevout.hash, tx, Params().GetConsensus(), hash, true)) {
        BOOST_FOREACH(CTxOut out, tx.vout)
            if(out.nValue == 5000*COIN && out.scriptPubKey == payee) return true;  //tbd
    }
...
void CMasternode::UpdateLastPaid(const CBlockIndex *pindex, int nMaxBlocksToScanBack)
{
    ...
    LogPrint("masternode","CMasternode::UpdateLastPaid nBlockLastPaid:%d nMaxBlocksToScanBack:%d \n",nBlockLastPaid,nMaxBlocksToScanBack);
...
bool CMasternodeBroadcast::CheckOutpoint(int& nDos)
{
...

    {
        ...
        if (err == COLLATERAL_INVALID_AMOUNT) {
            LogPrint("masternode", "CMasternodeBroadcast::CheckOutpoint -- Masternode UTXO should have 1000 BTR, masternode=%s\n", vin.prevout.ToStringShort());
            return false;
        }
    ...
    // should be at least not earlier than block when 1000 BTR tx got nMasternodeMinimumConfirmations
    ...
    GetTransaction(vin.prevout.hash, tx2, Params().GetConsensus(), hashBlock, true);
    {
        ...
        if (mi != mapBlockIndex.end() && (*mi).second) {
            CBlockIndex* pMNIndex = (*mi).second; // block for 1000 BTR tx -> 1 confirmation
