...
bool CMasternodeMan::GetNextMasternodeInQueueForPayment(int nBlockHeight, bool fFilterSigTime, int& nCountRet, masternode_info_t& mnInfoRet)
{
    ...
    if (!masternodeSync.IsWinnersListSynced()) {
        ...
        LogPrintf("!masternodeSync.IsWinnersListSynced \n");
        return false;
    }
    ...
    */
    ...
    for (auto& mnpair : mapMasternodes) {
        ...
        if(mnpair.second.nProtocolVersion < mnpayments.GetMinMasternodePaymentsProto())
            {
                LogPrintf("masternode!nProtocolVersion < mnpayments.GetMinMasternodePaymentsProto \n");
                continue;
            }
        ...
        {
            LogPrintf("!masternode IsScheduled \n");
            continue;
        }
        ...
        {
             LogPrintf("!masternode fFilterSigTime> \n");
            continue;
        }
        ...
        {
            LogPrintf("!masternode GetUTXOConfirmations> \n");
            continue;
        }
    ...
    uint256 blockHash;
    if(!GetBlockHash(blockHash, nBlockHeight - 20)) {
