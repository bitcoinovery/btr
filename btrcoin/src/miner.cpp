...
void IncrementExtraNonce(CBlock* pblock, const CBlockIndex* pindexPrev, unsigned int& nExtraNonce)
{
    ...
    //LogPrintf("IncrementExtraNonce: nHeight:%d nExtraNonce:%u scriptSig:%s coinBase:%s \n CScriptNum(nExtraNonce):%s\n",nHeight,nExtraNonce,HexStr(txCoinbase.vin[0].scriptSig),HexStr(COINBASE_FLAGS),HexStr(CScriptNum(nExtraNonce).getvch()));
...
// ***TODO*** ScanHash is not yet used in BTR
//
...
void static BitcoinMiner(const CChainParams& chainparams, CConnman& connman)
{
    ...
    try {
        ...
        while (true) {
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("BTRMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            ...
            LogPrintf("BTRMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
            ...
            while (true)
            {
                ...
                while (true)
                {
                    ...
                    //LogPrintf("BitcoinMiner: nNonce:%u \n",pblock->nNonce);
                    nHashesDone += 1;
    ...
    {
        LogPrintf("BTRMiner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("BTRMiner -- runtime error: %s\n", e.what());
        return;
    }
}
