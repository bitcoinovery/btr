...
void CSporkManager::ProcessSpork(CNode* pfrom, std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if (strCommand == NetMsgType::SPORK) {
        ...
        if(mapSporksActive.count(spork.nSporkID)) {
            ...
            LogPrint("spork","%s new\n", strLogMsg);
        }

        if(!spork.CheckSignature()) {
            LogPrint("spork","CSporkManager::ProcessSpork -- invalid signature\n");
...
bool CSporkManager::IsSporkActive(int nSporkID)
{
    ...
    if(mapSporksActive.count(nSporkID)){
        ...
        LogPrint("spork","IsSporkActive1:nValue %I64d \n",r);
    } else {
        switch (nSporkID) {
    ...
    LogPrint("spork","IsSporkActive:nValue %I64d GetAdjustedTime:%I64d \n",r,GetAdjustedTime());
