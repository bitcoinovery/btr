...
const char * const BITCOIN_CONF_FILENAME = "btr.conf";
const char * const BITCOIN_PID_FILENAME = "btrd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "btr" is a composite category enabling all BTR-related debug output
            if(ptrCategory->count(string("btr"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BTRCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BTRCore
    // Mac: ~/Library/Application Support/BTRCore
    // Unix: ~/.btrcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BTRCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BTRCore";
#else
    // Unix
    return pathRet / ".btrcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty btr.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override btr.conf
