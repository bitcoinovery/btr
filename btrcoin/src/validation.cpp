...
#if defined(NDEBUG)
# error "BTR Core cannot be compiled without assertions."
#endif
...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
   ...
   // CAmount nSubsidyBase;
    ...
    LogPrint("bench","PrevHeight:%d\n",nPrevHeight);
  //  nPrevHeight = nPrevHeight*100;
    if(nPrevHeight == 0)
    {
        return 2520000*COIN;
    }
    ...
    else if((nPrevHeight>172800) &&(nPrevHeight <= 348000))
    {

        return 25*COIN;
    }
    else if((nPrevHeight>348000) &&(nPrevHeight<=523200))
    {

        return 15*COIN;
    }
    else if((nPrevHeight>523200) &&(nPrevHeight<=698400))
    {

        return 10*COIN;
    }
    else if(nPrevHeight >698400  &&(nPrevHeight<= 21000000))   //2100万个结束
    {
...
//    LogPrintf("nSuperblockPart:%d nSubsidy:%d \n",nSuperblockPart,nSubsidy);
...
//    else if((nHeight>172800) &&(nHeight <= 348000))
//    {
//        nSubsidyBase = 25;
//    }
//    else if((nHeight>348000) &&(nHeight<=523200))
//    {
//        nSubsidyBase = 15;
//    }
//    else if((nHeight>523200) &&(nHeight<=698400))
//    {
//        nSubsidyBase = 10;
//    }
//    else if(nHeight >698400)
//    {
//        nSubsidyBase = 5;
//    }
...
static bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck = false)
{
    ...
    // BTR : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(BTR): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(BTR): couldn't find masternode or superblock payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // BTR : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(BTR): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(BTR): spork is off, skipping transaction locking checks\n");
    }
